...
```{r echo=FALSE}
ottr_template_sync <- read_yaml("https://raw.githubusercontent.com/ottrproject/OTTR_Template/refs/heads/main/.github/sync.yml")
```

```{r echo=FALSE}
sync_repos <- ottr_template_sync$group[[1]]$repos %>%
  str_trim() %>%
  as.data.frame() %>%
...
There are `r nrow(sync_repos)` GitHub repositories receiving updates from the OTTR Course Template. `r nrow(sync_repos %>% filter(org == "jhudsl" | org == "fhdsl"))` of those are within the jhudsl and fhdsl organizations.
...
```{r echo=FALSE}
ottr_web_sync <- read_yaml("https://raw.githubusercontent.com/ottrproject/OTTR_Template_Website/refs/heads/main/.github/sync.yml")
```

```{r echo=FALSE}
web_sync_repos <- ottr_web_sync$group[[1]]$repos %>%
  str_trim() %>%
  as.data.frame() %>%
  separate_longer_delim(cols = ".", delim = "\n") %>%
  separate(".", c("org", "repo"), sep = "/", remove = FALSE)
...
There are `r nrow(web_sync_repos)` GitHub repositories receiving updates from the OTTR Website Template. `r nrow(web_sync_repos %>% filter(org == "jhudsl" | org == "fhdsl"))` of those are within the jhudsl and fhdsl organizations.
...
loqui_data <- read_sheet("https://docs.google.com/spreadsheets/d/1G_HTU-bv2k5txExP8EH3ScUfGqtW1P3syThD84Z-g9k")
```

<!--
Currently data hasn't been and can't be added to this spreadsheet (as of July 2024)...Fixing the appending problem and investigating if we can estimate at least how many videos we don't have user data for
-->
